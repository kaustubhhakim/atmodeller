...
@dataclass(kw_only=True)
class FugacityModelABC_V2(GetValueABC):
    """A fugacity model. Version 2 for Saxena Data, provides better fit to the high P data

    This base class requires a specification for the ln(phi)
    ...
    def get_value(self, *, temperature: float, pressure: float) -> float:
        ...
        pressure /= self.scaling
        # fugacity_coefficient: float = self.fugacity_coefficient(temperature, pressure)
        fugacity_coefficient: float = self.fugacity_coefficient_v2(temperature, pressure)
    ...
    def fugacity_coefficient_v2(self, temperature: float, pressure: float) -> float:
        """Fugacity coefficient.
        Calculated using equation for ln(phi) from https://en.wikipedia.org/wiki/Fugacity
        ...
        Returns:
            fugacity coefficient (phi), dimensionless
        """
        fugacity_coefficient: float = np.exp(
            self.ln_fugacity_coefficient_fromint(temperature, pressure)
        )
